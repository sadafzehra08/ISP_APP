...
export class InvoiceDetail implements OnInit {
  invoice: Invoice | undefined;   // ← ! hata k | undefined karo
  ...
  private loadInvoice(id: number): void {
    // Replace with: this.invoiceService.getById(id).subscribe(...)
    this.invoice = {
      id: 1,
  ...
  edit(): void  { if (this.invoice?.id) this.router.navigate(['/invoices', this.invoice.id, 'edit']); }
